...
description: シニアデザイナー視点で UX 完全性・情報設計・アクセシビリティ・体験の差別化をレビューする。初期立ち上げ時は requirements.md まで、機能追加時は delta.md までできた段階で実行。引数で機能スラグを指定可、無指定時は WIP を自動検出。
...
レビュー対象（初期立ち上げ全体 or 特定機能の delta）について、UX / 情報設計観点での評価・ギャップ・推奨を `reviews/designer.md` に書き出す。
...
- 無指定で `docs/product/features/*/` 配下に `delta.md` あり `reviews/designer.md` なしの WIP があれば、それを候補として提示し確認
...
- 出力先: `docs/product/reviews/designer.md`

**Feature mode**:
- `proposal.md`, `delta.md` を Read
- コンテキスト: 親 `discovery.md`, `journey.md`, `requirements.md`, 過去 features の `brief.md`、`decisions/*.md` も Read
- 出力先: `docs/product/features/NNN-<slug>/reviews/designer.md`

### 3. 評価軸

以下 7 軸で 0-10 のスコアと根拠を書く（gstack の `/plan-design-review` を参考）。
...
対象パスに `reviews/designer.md` を Write。
...
- 対象の `reviews/designer.md` が書かれている
- 全 7 軸にスコアと根拠がある
